...
export const ReferralPage: React.FC<ReferralPageProps> = ({
  ...
  // Read state from referralService
  const [refData, setRefData] = useState<ReferralData>(() => referralService.getData());
  ...
  // Subscribe to referralService state
  useEffect(() => {
    const unsub = referralService.subscribe((newData) => {
      setRefData(newData);
    });
    return unsub;
  }, []);
  ...
  // Activate Sponsor Referral Code (One-time only -> $10 bonus)
  const handleActivateSponsor = (e: React.FormEvent) => {
    e.preventDefault();
    setSponsorError('');
    setSponsorSuccess('');

    const res = referralService.activateSponsor(sponsorInput, userName);
    if (!res.success) {
      sound.playLose();
      setSponsorError(res.message);
    } else {
      sound.playWin();
      setSponsorSuccess(res.message);
      setSponsorInput('');
    }
  };
  ...
  const handleConvertCommission = () => {
    ...
    setTimeout(() => {
      const res = referralService.convertToLive();
      setIsConverting(false);
      if (res.success) {
        onConvertToLive(res.amount);
        setConvertSuccessMsg(`✅ Successfully transferred $${res.amount.toFixed(2)} to your Live Real Balance!`);
        setTimeout(() => setConvertSuccessMsg(''), 6000);
      }
    }, 600);
  };

  // Accurate simulation helpers testing the 20% loss & win rule
  const handleSimulateTradeLoss = () => {
    sound.playWin();
    const tradeInvestment = 20.0;
    referralService.processTradeCommission(tradeInvestment, 'LOSS');
  };

  const handleSimulateTradeWin = () => {
    sound.playClick();
    const tradeAmount = 15.0;
    referralService.processTradeCommission(tradeAmount, 'WIN');
  };

  const handleAddSampleTrader = () => {
    sound.playClick();
    const sampleNames = ['trader_alex', 'crypto_rahim', 'forex_vip', 'bangla_trader'];
    const randomName = sampleNames[Math.floor(Math.random() * sampleNames.length)] + '_' + Math.floor(100 + Math.random() * 900);
    referralService.registerReferredTrader(randomName, 10.0);
  };
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#080d1a] text-slate-100 p-3 sm:p-6 md:p-8 select-none">
      <div className="max-w-5xl mx-auto space-y-6">
        {/* Top Header Banner with 3D Accents */}
          ...
          <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-5">
            <div>
              ...
              <p className="text-xs sm:text-sm text-slate-300 mt-1 max-w-xl leading-relaxed">
                Start with a clean <strong>$0.00</strong> commission balance. Activate a sponsor code for an instant{' '}
                <strong className="text-amber-400 font-bold">$10.00 bonus</strong>. Earn an automatic{' '}
                <strong className="text-emerald-400 font-bold">20% commission</strong> on referred trader losses, with 20% deducted on trade wins.
              </p>
            </div>
          ...
          <div className="lg:col-span-2 rounded-2xl bg-gradient-to-b from-[#131b2e] to-[#0d1424] border border-slate-700/80 p-5 sm:p-6 shadow-xl flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <span className="block text-[10px] text-slate-400 font-mono">
                      Starts at $0.00 • Instant $10 Sponsor Bonus on Activation
                    </span>
                  </div>
                </div>
                ...
                >
                  {canConvert ? 'READY TO CONVERT' : `NEED $${Math.max(0, MIN_CONVERT_THRESHOLD - commissionBalance).toFixed(2)} MORE`}
                </span>
              </div>
              ...
              <div className="mt-4">
                <div className="flex items-center justify-between text-xs font-mono mb-1.5">
                  <span className="text-slate-400">
                    Progress to Live Real Balance Conversion (Min $50.00)
                  </span>
          ...
          <div className="rounded-2xl bg-gradient-to-b from-[#161f34] to-[#0f172a] border border-amber-500/30 p-5 sm:p-6 shadow-xl flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <div>
                  ...
                  <span className="text-[10px] text-slate-400">Share to earn 20% from referred traders</span>
                </div>
              </div>

              {/* Big Display of Referral ID */}
              <div className="p-3.5 rounded-xl bg-[#0b101c] border border-amber-500/40 shadow-inner flex items-center justify-between">
                <div>
                  <span className="text-[10px] text-slate-400 font-mono uppercase block">Your Personal Code</span>
            ...
            <div className="mt-4 pt-3 border-t border-slate-800/80 grid grid-cols-2 gap-2 text-center font-mono">
              <div className="p-2 rounded-lg bg-[#0d1322]">
                ...
                <span className="text-sm font-black text-white">{referredUsers.length} Active</span>
              </div>
              <div className="p-2 rounded-lg bg-[#0d1322]">
        ...
        {/* ========================================================================= */}
        {/* ROW 2: ACTIVATE SPONSOR REFERRAL CODE ($10 BONUS) + 20% COMMISSION RULES   */}
        {/* ========================================================================= */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <span className="text-[10px] text-slate-400">
                    Get an instant <strong>$10.00</strong> bonus added to your referral balance
                  </span>
                </div>
              </div>
            ...
            {activatedSponsorCode ? (
              ...
              <div className="p-4 rounded-xl bg-[#090e1a] border border-emerald-500/40 mt-3 space-y-2">
                ...
                <div className="pt-2 border-t border-slate-800 text-[11px] text-amber-400 font-bold flex items-center gap-1.5">
                  ...
                  <span>$10.00 Welcome Bonus has been credited to your referral balance!</span>
                </div>
              </div>
            ) : (
              // Active Input Form
          ...
          {/* Card: 20% Referral Commission Rules (Loss 20%, Win 20% Reduction) */}
          <div className="rounded-2xl bg-gradient-to-b from-[#131b2e] to-[#0c1220] border border-slate-700/80 p-5 sm:p-6 shadow-xl flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <div>
                  <h3 className="text-xs font-black text-slate-200 uppercase tracking-wider">
                    20% Commission & Profit Offset Rules
                  </h3>
                  <span className="text-[10px] text-slate-400">Strict mathematical transparency</span>
                </div>
              </div>
              ...
              <div className="space-y-2.5 text-xs text-slate-300">
                <div className="p-2.5 rounded-xl bg-[#090e1a] border border-slate-800 flex items-start gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400 mt-0.5">
                      When a referred trader loses a $20 trade, <strong>+$4.00 (20%)</strong> is automatically added to your referral commission balance.
                    </p>
                  </div>
                </div>
                ...
                <div className="p-2.5 rounded-xl bg-[#090e1a] border border-slate-800 flex items-start gap-2.5">
                  ...
                  <div>
                    <strong className="text-white">Trader Win → 20% Auto Reduced from Balance</strong>
                    <p className="text-[11px] text-slate-400 mt-0.5">
                      When a referred trader wins a trade, <strong>20% of the trade amount</strong> is automatically reduced from your referral balance (balance cannot drop below $0.00).
                    </p>
                  </div>
                </div>
                ...
                <div className="p-2.5 rounded-xl bg-[#090e1a] border border-slate-800 flex items-start gap-2.5">
                  ...
                  <div>
                    <strong className="text-white">Instant $10 Sponsor Bonus</strong>
                    <p className="text-[11px] text-slate-400 mt-0.5">
                      New accounts start at $0.00. Activating any friend's sponsor code immediately adds $10.00 to your referral balance.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-800/80 flex flex-wrap items-center justify-between gap-2">
              <span className="text-[10px] text-slate-400 font-mono">Commission Test Simulator:</span>
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  ...
                  title="Simulate $15 win by referred trader (-20% = -$3.00 reduction)"
                >
                  - Simulate $15 Win (-20%)
                </button>
              </div>
        ...
        {/* ========================================================================= */}
        {/* ROW 3: REFERRED USERS LIST TABLE (ACCURATE REAL DATA ONLY)                 */}
        {/* ========================================================================= */}
        <div className="rounded-2xl bg-gradient-to-b from-[#131b2e] to-[#0c1220] border border-slate-700/80 p-5 sm:p-6 shadow-xl">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-[10px] text-slate-400">
                  Accurate live data of traders who registered via your referral code
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={handleAddSampleTrader}
                className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white text-[10px] font-bold border border-slate-700 flex items-center gap-1 transition-colors cursor-pointer"
                title="Add a test referred trader to the network"
              >
                <UserPlus className="w-3 h-3 text-cyan-400" />
                <span>+ Add Referral Trader</span>
              </button>
              <span className="text-[10px] text-slate-400 font-mono hidden sm:inline">Net Commission:</span>
          ...
          {referredUsers.length === 0 ? (
            <div className="p-8 sm:p-12 text-center rounded-xl bg-[#090e1a]/80 border border-slate-800/80 flex flex-col items-center justify-center">
              ...
              <h4 className="text-sm font-bold text-slate-200 mb-1">
                No Referred Traders in Your Network Yet
              </h4>
              <p className="text-xs text-slate-400 max-w-md leading-relaxed mb-4">
                All fake demo data has been removed. Share your referral link or code with other traders. When they register and trade, their live activity and 20% commission will appear here automatically.
              </p>
              <div className="flex flex-wrap items-center justify-center gap-2">
                <button
                  onClick={handleCopyLink}
                  className="px-4 py-2 rounded-xl bg-gradient-to-r from-amber-500 to-yellow-400 hover:from-amber-400 hover:to-yellow-300 text-slate-950 font-black text-xs flex items-center gap-1.5 shadow-md active:scale-95 transition-all cursor-pointer"
                >
                ...
                >
                  ...
                  <span>Simulate Real Referral Registration</span>
                </button>
              </div>
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-slate-800/60 font-mono">
                  {referredUsers.map((user) => (
                    <tr key={user.id} className="hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className={`py-3 px-3 font-black ${user.commissionGenerated >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {user.commissionGenerated >= 0 ? `+$${user.commissionGenerated.toFixed(2)}` : `-$${Math.abs(user.commissionGenerated).toFixed(2)}`}
                      </td>
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-[10px] text-slate-400">Automatic 20% loss credits & 20% win reductions</span>
              </div>
            </div>
          </div>

          {commissionLogs.length === 0 ? (
            <div className="p-6 text-center text-xs text-slate-500 font-mono bg-[#090e1a]/60 rounded-xl border border-slate-800">
              No referral transactions yet. Transactions will be logged here as referral bonuses and commissions occur.
            </div>
          ) : (
            <div className="space-y-2">
              {commissionLogs.slice(0, 8).map((log) => {
